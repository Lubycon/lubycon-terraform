...
resource "aws_route_table" "database" {
  ...
  tags = merge(var.tags, var.database_route_table_tags, map("Name", "${var.name}-db-rt"))
}
...
resource "aws_subnet" "database" {
  ...
  tags = merge(map("Name", format("%s.db-sub.%s", var.name, element(var.azs, count.index))), var.database_subnet_tags, var.tags)
}

resource "aws_db_subnet_group" "database" {
  count = var.create_vpc && length(var.database_subnets) > 0 && var.create_database_subnet_group ? 1 : 0
  name        = format("%s-%s-%s-db-sub-group", element(split("-",var.name),0),element(split("-",var.name),1),element(split("-",var.name),2))
  description = "Database subnet group for ${var.name}"
  subnet_ids  = aws_subnet.database.*.id

  tags = merge(map("Name", format("%s-db-sub-group", var.name)), var.database_subnet_group_tags, var.tags)
}
...
## Redis SubnetGroup, DB Subnet에 포함
resource "aws_elasticache_subnet_group" "elasticache" {
